...
class VidoeApi {
  FutureEither<SearchResultDtoVideos> searchVideos(
      {required SearchQueryModel query}) async {
    // try {
    final response = await AppNetwork.get(
        url: AppApi.baseUrl + AppApi.searchVideo,
        queryParameters: query.toQuery());

    return response.fold(
      (error) => Left(ServerFailure(message: error)),
      (result) => Right(
        SearchResultDtoVideos.fromJson(result.data),
      ),
    );
    // } catch (e) {
  ...
  FutureEither<SearchResultDtoVideos> getPopularVideos(
      {required int page}) async {
    try {
      final response = await AppNetwork.get(
          url: AppApi.baseUrl + AppApi.searchPopular,
